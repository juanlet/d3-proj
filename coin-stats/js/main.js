...
$(document).ready(function () {
    ...
    var xAxisCall = d3.axisBottom().ticks(4).tickFormat(function (d) {
        if (moment(dateRangeSelected[0]).format("YYYY") === moment(dateRangeSelected[1]).format("YYYY") ||
            parseInt(moment(dateRangeSelected[0]).format("YYYY")) === parseInt(moment(dateRangeSelected[1]).format("YYYY")) - 1) {
            //if selector is on the same year or  one year apart display month with year 

            return moment(d).format("MMM - YY");
        }
        //if selector is on different years it will display years      
